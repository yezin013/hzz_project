...
def get_secret(key: str, default: Optional[str] = None) -> Optional[str]:
    ...
    Args:
        key: 시크릿 키 (예: "MONGODB_PASSWORD")
        default: 기본값
    
    Returns:
        시크릿 값 또는 기본값
    """
    # 1. 환경변수 우선 확인 (로컬/EKS 또는 오버라이드)
    env_value = os.getenv(key)
    if env_value is not None:
        return env_value
    
    # 2. Secrets Manager에서 확인 (Lambda)
    secrets = get_secrets()
    if key in secrets:
        return secrets[key]
...
def get_mongodb_config() -> Dict[str, str]:
    ...
    return {
        ...
        "db": get_secret("MONGODB_DB", "admin"),
        "replica_set": get_secret("MONGODB_REPLICA_SET", "rs0"),
        "url": get_secret("MONGO_URL", ""),
    }
...
def get_elasticsearch_config() -> Dict[str, str]:
    ...
    return {
        "hosts": get_secret("ELASTICSEARCH_HOSTS", "localhost"),
        "port": get_secret("ELASTICSEARCH_PORT", "9200"),
        "username": get_secret("ELASTICSEARCH_USERNAME", "elastic"),
        "password": get_secret("ELASTICSEARCH_PASSWORD", ""),
    }
